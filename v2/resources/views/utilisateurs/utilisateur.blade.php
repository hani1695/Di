                ...
                <div class="collapse {{ Session::has('errors') ? 'show' : ''}}" id="collapseExample">
                    <div class="card card-body">
                        <form action="{{ route('register') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div id="app2">
                                {{-- <dr-agence nomdr="{{ old('nom_dr') }}" st="{{ old('structure') }}" org="{{ old('organisme') }}" nomdrerror="@error('nom_dr'){{ $message }}@enderror" sterror="@error('structure'){{ $message }}@enderror" orgerror="@error('organisme'){{ $message }}@enderror" /> --}}
                                <org-dir-stru  nomdr="{{ old('nom_dr') }}" str="{{ old('structure') }}" org="{{ old('organisme') }}" nomdrerror="@error('nom_dr'){{ $message }}@enderror" sterror="@error('structure'){{ $message }}@enderror" orgerror="@error('organisme'){{ $message }}@enderror" ></org-dir-stru>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control @error('profile') is-invalid @enderror" id="exampleFormControlSelect1" name="profile">
                                  ...
                                  @foreach ($profiles as $data)
                                    <option value="{{ $data->code }}">{{ $data->limitation.'-'.$data->code.'-'.$data->libelle }}</option>                                  
                                  @endforeach            
                                </select>     
                                @error('profile')
                                    <span class="invalid-feedback" role="alert">
...
{{--<div>
    <form action="{{route('utilisateur.create')}}" class="container">
        <button type="submit" class="btn btn-info boutonajout" > Ajout un utilisateur</button>
    </form> --}}
    
    {{-- <form action="{{route('utilisateur.index')}}" class="container">
